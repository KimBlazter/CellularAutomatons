...
public class DynamicGrid extends Application {
    ...
    private static final int GRID_WIDTH = 500;  // Nombre de colonnes
    private static final int GRID_HEIGHT = 500; // Nombre de lignes
    ...
    private int countNeighbors(int x, int y, int radius) {
        ...
        for (int i = -radius; i <= radius; i++) {
            for (int j = -radius; j <= radius; j++) {
                if (checkGridBounds(x + i, y + j) && !(i == 0 && j == 0)) {
                    if (grid[x + i][y + j] == 1) counter++;
                }
            }
        }
        return counter;
    ...
    private void handleMouseDraw(MouseEvent event) {
        ...
        if (checkGridBounds(gridX, gridY)) {
            grid[gridX][gridY] = event.isPrimaryButtonDown() ? 1 : 0; // 1 if left click and 0 if right click
            drawGrid(canvas.getGraphicsContext2D());
        }
